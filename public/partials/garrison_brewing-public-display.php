 ...
 * @subpackage Garrison_brewing/public/partials
 */
...
?>
    <div class="flex flex-wrap -mx-4" style="max-width: 85%;">
        <div class="w-full px-4">
            <div class="mb-12 lg:mb-20 max-w-[620px]" style="max-width: 100%;">
                <?php
                if($response_body->beer_label)
                {
                    echo '<img src="'.esc_url_raw($response_body->beer_label).'">';
                }

                if($response_body->beer_name)
                {
                    echo '<h2 class="font-bold text-3xl sm:text-4xl md:text-[42px] text-dark mb-4">'.sanitize_text_field(__($response_body->beer_name),'garrison_brewing').'</h2>'; 
                }

                if($response_body->brewery->brewery_name)
                {
                    echo '<span class="font-semibold text-lg text-primary mb-2 block">
                ...
                {
                    echo '<p class="text-lg sm:text-xl leading-relaxed sm:leading-relaxed text-body-color">'.sanitize_text_field(__($response_body->beer_style,'garrison_brewing')).'</p>'; 
                }

                if($response_body->beer_abv)
                {
                   echo '<p class="text-lg sm:text-xl leading-relaxed sm:leading-relaxed text-body-color">'.sanitize_text_field(__($response_body->beer_abv,'garrison_brewing')).' '.__('% ABV','garrison_brewing').'</p>';
                }

                if($response_body->beer_ibu)
                {
                    echo '<p class="text-lg sm:text-xl leading-relaxed sm:leading-relaxed text-body-color">'.sanitize_text_field(__($response_body->beer_ibu,'garrison_brewing')).' '.__('IBU','garrison_brewing').'</p>';
                }
                ...
                if($checkins->count > 0 && $checkins->items)
                {
                    ...
                    echo ' ('.$avg_rating.'/5) ('.__('10 Reviews','garrison_brewing').')';
                }

                if($response_body->beer_description)
                {
                    echo '<p class="text-lg sm:text-xl leading-relaxed sm:leading-relaxed text-body-color">'.sanitize_text_field(__($response_body->beer_description,'garrison_brewing')).' IBU</p>';
                }

                if($response_body->media->count > 0)
                {
                    echo '<div class="w-full mx-4">
                            ...
                            foreach($items as $item)
                            {
                                ...
                                if($beer_img)
                                {
                                    echo '<div class="ud-single-logo mr-10 mb-5 max-w-[140px]">
                ...
                {  
                    ...
                    echo '<section id="testimonials" class="pt-20 md:pt-[120px]" style="    max-width: 100%;">
                                            ...
                                            <div class="ud-single-testimonial p-8 bg-white mb-12 shadow-testimonial wow fadeInUp" data-wow-delay=".1s" style="visibility: visible; animation-delay: 0.1s;">
                                                <div class="ud-testimonial-content mb-6">
                                                    ...
                                                    if($checkins_item->rating_score > 0){
                                                        echo ' '.$checkins_item->rating_score.'</p>';   
                                                    }

                                                    if($checkins_item->checkin_comment)
                                                    {
                                                        echo '<p class="text-base tracking-wide text-body-color">'.sanitize_text_field(__($checkins_item->checkin_comment,'garrison_brewing')).'</p>';
                                                    }
